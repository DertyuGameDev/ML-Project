...
from Models.LinerModel_1 import model as model_data
train_dl, test_dl = get_dataloader(transform=model_data.transform, batch=128)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(device)

model = model_data.Model()

show_metrics(*train_and_evaluate(model, train_dl, test_dl, 3, lr=1E-1))
